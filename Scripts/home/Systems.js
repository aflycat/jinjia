...
//获取配置列表
function treeConf() {
    var _url = "/GWServices.asmx/ConfList";
    ...
    var _urlAlarm = "/GWServices.asmx/GainAlarmTab";
...
function jsonToConf(data) {
    ...
    for (var i = 1; i < usera.length; i++) {
        var userb = usera[i];
        var userc = new Array(userb.name, userb.value, false);
        var newRow = $("<li></li>");
...
function treelistAll(event) {
    if (document.getElementById("treelistAll").checked) {
        for (var i = 1; i < confArry.length; i++) {
...
function SelectEquipData(tableName, opertions) {
    var _url = "/GWServices.asmx/SelectEqpYcpYxpConf";
    var _data = "";
    if (tableName == "Equip") {
        _data = "tableName=" + tableName + "&&counts=10&&pages=" + nowPage[0] + "&&select_id=" + selectEquip_Now + "&&isCount=false";
        var nowPageGet = document.getElementById("nowPage_now_1");
        nowPageGet.innerHTML = nowPage[0];
        equipTable = new Array();
    }
    else if (tableName == "ycp") {
        _data = "tableName=" + tableName + "&&counts=10&&pages=" + nowPage[1] + "&&select_id=" + selectEquip_Now + "&&isCount=false";
        var nowPageGet = document.getElementById("nowPage_now_2");
        nowPageGet.innerHTML = nowPage[1];
        ycpTable = new Array();
    }
    else if (tableName == "yxp") {
        _data = "tableName=" + tableName + "&&counts=10&&pages=" + nowPage[2] + "&&select_id=" + selectEquip_Now + "&&isCount=false";
        var nowPageGet = document.getElementById("nowPage_now_3");
        nowPageGet.innerHTML = nowPage[2];
        yxpTable = new Array();
    }
    else {
        _data = "tableName=" + tableName + "&&counts=10&&pages=" + nowPage[3] + "&&select_id=" + selectEquip_Now + "&&isCount=false";
...
function JSONToService_SYS(updateJSON, tableName) {
    if (updateJSON != null) {
        ...
        var _url = "/GWServices.asmx/UpdateSystemSetParm";
        var _data = "dataStr=" + jsonStr + "&&tableName=" + tableName;
        function _successf(data) {
            var resultJs = $(data).children("string").text();
            if (resultJs == "true") {
