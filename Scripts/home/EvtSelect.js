...
function treelistAll(event) {
    if (document.getElementById("treelistAll").checked) {
        for (var i = 1; i < numTree.length; i++) {
...
function tableData(tableName) {
    var _url = "/GWServices.asmx/EqpSetSysEvt";
    ...
    if (tableName == "EqpEvt") {
        ...
        _data = "tableName=" + tableName + "&&wheres=" + datatime + "&&selectStr=" + selectNums;
        ...
        $('#tableSys tbody').html("");
        _data = "tableName=" + tableName + "&&wheres=" + datatime + "&&selectStr=\"\"";
