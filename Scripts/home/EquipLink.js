...
//初始化数据
function allData() {
    var _url = "/GWServices.asmx/TableToJsonAlls";
    var _dataAll = "tableName=Equip,SetParm,ycp,yxp,AutoProc";
...
//保存场景列表
function save_scene() {
    var _url = "/GWServices.asmx/sceneSaveTab";
    //普通任务列表
    var datafrom = "";
    for (var i = 0; i < SetParmTable.length; i++) {
        if (SetParmTable[i][3] == "J") {
    ...
    var _data = "tabName=SetParm&&data=" + datafrom;
    function _success_1(data) {
        var resultJs = $(data).children("string").text();
        if (resultJs == "true") {
            alert("保存成功！");
