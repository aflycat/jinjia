function smartCom(){
	$("#smartCom .touchBtn").each(function(){
		$(this).off().click(function(){
			...
			if(hasDown){
				...
				$("#loading").show()
				GetSetParmItemSma(eq_no,set_no)
...
function GetSetParmItemSma(eq_no,set_no){
	$.ajax({
    type: "POST",
...
//      		console.log(eq_no,main_ins,minor_ins,value)
				SetupsCommandSma(eq_no,main_ins,minor_ins,value)
        });
    },
	    complete:function(){
	    	$("#loading").hide();
	    	$("#smartCom .touchBtn").removeClass("bgDown").find("p,i").css({color:"#34cbbd"});
	    }

});
}
function SetupsCommandSma(eq_no,main_ins,minor_ins,value){
	$.ajax({
	    type: "POST",
