...
function ManageEnergy(){
	$("#ManageEnergy .touchBtn").each(function(){
		$(this).off().click(function(){
			...
			if(hasDown){
				...
				GetSetParmItemMan(eq_no,set_no)
			}
		})
	})
...
function GetSetParmItemMan(eq_no,set_no){
	$.ajax({
    type: "POST",
...
//      		console.log(eq_no,main_ins,minor_ins,value)
				SetupsCommandMan(eq_no,main_ins,minor_ins,value)
        });
    },
	    complete:function(){
	    	$("#loading").hide()
	    	$("#ManageEnergy .touchBtn").removeClass("bgDown").find("p,i").css({color:"#34cbbd"});
	    }
});
}
function SetupsCommandMan(eq_no,main_ins,minor_ins,value){
	$.ajax({
	    type: "POST",
