...
function ZCCallBack(zichanID, imgID) {
    var _url = "/GWServices.asmx/GetImageFromSvr";
...
function zcSetParm(dt1, dt2) {
    ...
    zcToService(dt1, dt2, newRow);
}
function zcToService(dt1, dt2, newRow) {
    var imgByte = '';
    $('#' + dt2).find('.addimgListItme').each(function () {
        if ($(this).find('img').length > 0) {
            imgByte += $(this).find('img').attr('src').split(',')[1] + ',';
        }
    });
    imgByte = imgByte.substring(0, imgByte.length - 1);
    var _urlUpload = "/GWServices.asmx/AddWeihuRecord4ZC";
    var _dataUpload = {
        ziChanID: newRow[0],
        name: newRow[1],
        recordStr: newRow[2],
    ...
    JQajaxo("post", _urlUpload, true, _dataUpload, _callBackUpload);
    function _callBackUpload(data) {
        var resultJs = $(data).text();
        if (resultJs != "false" && resultJs != "") {
            console.log('图片上传成功！');
            $('#' + dt1).find('ul').find('img').attr('src', '/Image/alarm/CommunicationOK.png');
            myApp.modal({
                title: "",
                text: '维护成功！',
                buttons: [
