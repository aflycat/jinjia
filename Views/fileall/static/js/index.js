...
function aniShow(stArr) {
    ...
    ctx.globalCompositeOperation = 'lighter';//source-atop
    for (var i = 1, l = stArr.length; i < l; i++) {
        stArr[i].draw(2, 0, -1.2);
    };
   ...
   // $("#circle").css({'transform':$('#circle').css('transform')});  
    ...
    flyto.addEventListener("play",function(){
      i = window.setInterval(function() {
        ...
        if(currentTime >= (duration*0.95) && noinfinite08) //二级页面动画加载
        {
            noinfinite08 = false;$('#mv-flyto').animate({'opacity': '0.5'}, 1000);
            $(".backgroundAll").css('z-index',98).animate({'opacity':'1'}, 1500);
        ...
        if(currentTime==duration)  //取消监听视频进度
         {
            ...
            $("body").append(html);
            selectOpation(myChart,busLines,busLines1,10,50,4500,7500,0.8,true);
            setTimeout(function(){
